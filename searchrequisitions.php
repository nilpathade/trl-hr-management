        ...
        <table class="table">
            ...
            <?php
              ...
              if(count($resultRow) > 0 ){
                ...
                foreach ($resultRow as $rows) {
                  $applySql = "select req.*, aj.status from tlr_requisitions as req INNER JOIN tlr_applyjob as aj ON req.id = aj.requisitionId where req.isDeleted= ? AND aj.applicantId = ? AND req.id = ?";
                  $applyresult = pdoQuery($applySql,array('N', $userid, $rows['id']));
                  ...
                  <tr>
                      ...
                      <td>
                        <?php if(empty($applyresult)) { ?>
                          <a href="#" onclick="javascript:applyjob(<?php echo $rows['id']; ?>, <?php echo $userid; ?>)"> Apply </a> 
                        <?php } else  {
                          ?>
                           <span style="color:green; font-style: italic;"> Applied </span>
                          <?php 
                        } ?>
                      </td>
                  </tr>
