...
$totalSql = "select * from tlr_sourcingteam where isDeleted= ? and roleType= ?  $condition";
$totalResult = pdoQuery($totalSql, array('N', 1));
...
$regeditSql = "select * from tlr_sourcingteam where isDeleted= ? and roleType= ? $condition limit " . $pageno . ',' . $limit;
$resultRow = pdoQuery($regeditSql, array('N', 1));
if (!empty($message)) echo $message;
if ($sourceAccess == 3) {
?>
<div class="col-md-3"><a href="sourceregistration.php"><button class="custom-btn mb-3">Add New Source</button></a></div>
<?php  } ?>
<div class="row">
    ...
    <div class="col-md-3 mb-3">
        <input placeholder="Type something..." type="text" name="filterValue" class="form-control" id="filterValue"
    ...
    <table class="table mt-3">
        <thead class="custom-thead">
            <tr>
                ...
                <?php if ($sourceAccess == 3) { ?> <th scope="col">Actions</th> <?php  } ?>
            </tr>
        </thead>
        <tbody>
            <?php
      ...
      if (count($resultRow) > 0) {
        ...
        foreach ($resultRow as $rows) {
          ...
          $rolename = getRoleName(1, $rows['role']);
      ?>
            <tr>
                ...
                <?php if ($sourceAccess == 3) { ?> <td><a class="icon"
                        href="sourceeditregistration.php?eid=<?php echo encrypt_decrypt('encrypt', $rows['id']); ?>"
                        title="edit"><i class="fas fa-edit bor-right"></i></a><a class="icon" href="#"
                        onClick="javascript:deletesourceconfirmation('<?php echo $sourceId; ?>,<?php echo $delete; ?>');"
...
<tr>
    <td colspan="7">
        <nav aria-label="Page navigation example">
          ...
          if ($pageno > 0) { ?>
                <li class="page-item">
                    <a href="sourcegridregistration.php?pageno=1&fixlimit=<?php echo $fixlimit; ?>" class="page-link">
                        Previous </a>
                </li>
                <?php   }
          for ($i = 0; $i < $totalPages; $i++) {  ?>
                <li class="page-item"> <a
                        href="sourcegridregistration.php?pageno=<?php echo $i + 1; ?>&fixlimit=<?php echo $fixlimit; ?>"
                        class="page-link"><?php echo $i + 1; ?></a></li>
                <?php }
          if ($totalPages > $pageno + 1) { ?>
                <li class="page-item"> <a
                        href="sourcegridregistration.php?pageno=<?php echo $totalPages; ?>&fixlimit=<?php echo $fixlimit; ?>"
