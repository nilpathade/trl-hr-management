...
if($registration == md5("tlraddsourcesubmit")){
	...
	$sctRow = pdoQuery($sctSql,array('N',1));
	if(count($sctRow) == 0){
		$sctId = 'S001';
	}else{
		$sctIdArr = explode('S',$sctRow[0]['sctId']);
		$sctId = $sctIdArr[1]+1;
	}
	...
	if(count($checkRow)==0){
	    $insertSql = "insert into tlr_sourcingteam (`sctId`,`firstName`, `lastName`, `email`, `password`, `mobile`, `currentDesignation`, `currentDepartment`, `currentCompany`, `status`, `role`, `roleType`, `masterPassword`, `isDeleted`) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
		$resultRow = pdoQuery($insertSql,array('S'.$sctId,$firstname,$lastname,$email,$password,$phonenumber,$currentDesignation,$currentDepartment,$currentCompany,$status,$role,1,$masterPassword,'N'));
		if($resultRow){
			$message = "success";
			}else{
				$message = "error";
	...
	header('location:sourceregistration.php?message='.md5($message));

}else if($registration == md5("sourceeditsubmit")){
