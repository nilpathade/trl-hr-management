...
if (!empty($filterColumn) && !empty($filterValue)) {
  if ($filterColumn == 'stageName') {
    $condition = "and $filterColumn like '%$filterValue%'";
    $selected = "selected = 'selected'";
  }
}
$totalSql = "select * from tlr_selectionstages where isDeleted= ? $condition";
...
$regeditSql = "select * from tlr_selectionstages where isDeleted= ? $condition limit " . $pageno . ',' . $limit;
$resultRow = pdoQuery($regeditSql, array('N'));
if (!empty($message)) echo $message;
if ($stagesAccess == 3) {
?>
<div class="col-md-3"><a href="stageaddmaster.php"><button class="custom-btn mb-3">Add New Stage</button></a></div>
<?php } ?>
<div class="row">
    ...
    <div class="col-md-2 mb-3">
        <form method="POST" id="filterform" name="filterform">
            <select name="filterColumn" class="form-select" id="filterColumn">
                ...
                <option value="stageName" <?php echo $selected; ?>>Stage Name</option>
            </select>
    </div>
...
<div class="responsive-table">
    <table class="table mt-3">
        <thead class="custom-thead">
            <tr>
                <th scope="col">#</th>
                <th scope="col">Requisitions Name</th>
      ...
      if (count($resultRow) > 0) {
        ...
        foreach ($resultRow as $rows) {
          $stageId = encrypt_decrypt('encrypt', $rows['id']);
          $requisitionName = getRequisitionName($rows['reqId']);
      ?>
            <tr>
                <th scope="row"><?php echo $k; ?></th>
                <td><?php echo $requisitionName; ?></td>
                <td><?php echo $rows['stageName']; ?></td>
                <?php if ($stagesAccess == 3) { ?> <td><a class="icon"
                        href="stageeditmaster.php?eid=<?php echo encrypt_decrypt('encrypt', $rows['id']); ?>"
                        title="edit"><i class="fas fa-edit bor-right"></i></a><a class="icon" href="#"
                        onClick="javascript:deletestageconfirmation('<?php echo $stageId; ?>');" title="Delete"><i
...
<tr>
    <td colspan="7">
        <nav aria-label="Page navigation example">
          ...
          if ($pageno > 0) { ?>
                <li class="page-item">
                    <a href="stagegridmaster.php?pageno=1&fixlimit=<?php echo $fixlimit; ?>" class="page-link">
                        Previous </a>
                </li>
                <?php   }
          for ($i = 0; $i < $totalPages; $i++) {  ?>
                <li class="page-item"> <a
                        href="stagegridmaster.php?pageno=<?php echo $i + 1; ?>&fixlimit=<?php echo $fixlimit; ?>"
                        class="page-link"><?php echo $i + 1; ?></a></li>
                <?php }
          if ($totalPages > $pageno + 1) { ?>
                <li class="page-item"> <a
                        href="stagegridmaster.php?pageno=<?php echo $totalPages; ?>&fixlimit=<?php echo $fixlimit; ?>"
